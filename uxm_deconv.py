...
def load_pats_homog(atlas, pats, tmp_dir, verb, rlen, force, ss_rate):
    pats = [op.abspath(p) for p in pats]
    uxm_dict = gen_homogs(atlas, pats, tmp_dir, verb, rlen, force)
...
def plot_counts(args, counts, atlas):
    if getpass.getuser() != 'nloyfer':
        return
    # breakpoint()
    counts.loc[atlas.iloc[:,2:].isnull().any(1).values] = np.nan
...
def main():
    ...
    params = [(sf[['name', 'direction', samp]], atlas[['name', 'direction'] + ref_cells], counts[['name', 'direction', samp]],
                args.weighted, args.verbose, args.debug) for samp in sample_names]
    if args.debug:
    ...
    if args.counts:
        plot_counts(args, counts.iloc[:, 1:], atlas[['name', 'target', 'direction'] + ref_cells])
...
def parse_args():
    ...
    parser.add_argument('--weighted', '-w', action='store_true',
            help='weight each marker by the number of reads it contains in the input sample')
    ...
    # parser.add_argument('--plot', action='store_true',
            # help='if set, show output figure in a popup window')
    # parser.add_argument('--png', action='store_true',
            # help='if set, save a png figure')
    # parser.add_argument('--plot_stubs', nargs='+',
            # help='For plotting: show only reference cell types that match any of the stubs')
    # parser.add_argument('--plot_nocollapse', action='store_true',
            # help='For plotting: Do not collapse cell types with <1 percent contribution')
    # parser.add_argument('--plot_full_legend', action='store_true',
